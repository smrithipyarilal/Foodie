...
console.log(firebase.auth().currentUser.email);
  ...
  form.addEventListener('submit', (e) => {
    ...
    var docRef = db.collection("MyData").doc(form.name.value);

    docRef.get().then(function(doc) {
    if (doc.exists) {
        console.log('Named document exists');

        //Removing old instance of restaurant data 
        db.collection("MyData").doc(form.name.value).delete().then(function() {
            console.log("Document successfully deleted!");
        });

        //Creating new Instance of data
        console.log('Creating new instance of document')
        db.collection('MyData').doc(form.name.value).set({
